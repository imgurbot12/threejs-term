...
import { FPSCounter, MemCounter } from "./counters.mjs";
import { polyfill } from "./dom_polyfill.mjs";
import { cube, objects, scene, sphere } from "./scene.mjs";
...
function init() {
  ...
  global.camera = new THREE.PerspectiveCamera(70, width / height, 1, 1000);
  ...
  global.renderer = new TerminalRenderer(canvas);
  ...
  function onResize(res) {
    ...
    screen.debug(
      `Estimated font size ${fontWidth.toFixed(3)}x${
        fontHeight.toFixed(3)
      }, ratio ${y_scale.toFixed(3)}`,
    );
    ...
    const height = screen.width * actual_screen_ratio;
...
setInterval(() => {
  ...
  const dataset = [fpsCounter.fps, memCounter.data // , fpsCounter.ms
  ];
